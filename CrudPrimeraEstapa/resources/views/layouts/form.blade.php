<fieldset>
    ...
    <div class="form-group m-3">
        <label class="text-white">
            {{ __('traduccion.Last name') }}: <input class="border-2 text-dark border-solid border-gray-100" type="text"
                name="apellido" value="{{ old('apellido', $usuario->apellido) }}" required />
            @error('apellido')
                <br>
                <small class="text-danger">*{{ $message }}</small>
                <br>
    ...
    <div class="form-group m-3">
        <label class="text-white">
            {{ __('traduccion.Phone') }}: <input class="border-2 text-dark border-solid border-gray-100" type="tel"
                name="telefono" value="{{ old('telefono', $usuario->telefono) }}" required />
            @error('telefono')
                <br>
                <small class="text-danger">*{{ $message }}</small>
                <br>
    ...
    <div class="form-group m-3 text-white">
        ...
        <label class="ml-1"> Si
            <input class="border-2 text-dark border-solid border-gray-100" type="radio" value="1" name="favicon" id=""
                {{ old('favicon', $usuario->favicon) == '1' ? 'checked=' . 'checked' : '' }}>
        </label>
        </label> No
    ...
    <div class="form-group m-3 ">
        <label class="text-white">{{ __('traduccion.Image') }}: 
            <input type="file" class="form-control-file" name="imagen" value="{{ old('imagen', $usuario->imagen) }}">
            @error('imagen')    
                <br>
                <small class="text-danger">*{{ $message }}</small>
                <br>
